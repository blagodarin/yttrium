...
BOOST_AUTO_TEST_CASE(test_intensity_to_bgra)
{
	...
	BOOST_CHECK(!intensity_to_bgra(bgra_image));
}
...
BOOST_AUTO_TEST_CASE(test_swap_channels)
{
	BOOST_TEST_CONTEXT("Gray")
	{
		const std::array<uint8_t, 8> gray
		{
			0x00, 0x01, 0x02, 0x03,
		...
		BOOST_REQUIRE(gray_image.swap_channels());
		BOOST_CHECK(!std::memcmp(gray_image.data(), gray.data(), gray.size()));
	}
	BOOST_TEST_CONTEXT("Rgb24 and Bgr24")
	{
		const std::array<uint8_t, 24> rgb
		{
			0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
		...
		BOOST_REQUIRE(rgb_image.swap_channels());
		BOOST_CHECK(!std::memcmp(rgb_image.data(), bgr.data(), bgr.size()));

		Image bgr_image({ 3, 2, PixelFormat::Bgr24, 4 }, bgr.data());
		BOOST_REQUIRE(bgr_image.swap_channels());
		BOOST_CHECK(!std::memcmp(bgr_image.data(), rgb.data(), rgb.size()));
	}
}
