...
{
	const auto actual = Yt::Matrix4::camera({ 1, 2, 3 }, { 30, 45, 60 });
	const auto expected = Yt::Matrix4::rotation(60, { 0, -1, 0 }) * Yt::Matrix4::rotation(45, { -1, 0, 0 }) * Yt::Matrix4::rotation(30, { 0, 0, 1 }) * Yt::Matrix4::translation({ -1, -2, -3 });
...
TEST_CASE("matrix4.det")
{
	...
	CHECK(det(Yt::Matrix4::identity()) == 1.f);
	CHECK(det(Yt::Matrix4::scaling(2)) == 2.f * 2.f * 2.f);
	CHECK(det(Yt::Matrix4::translation({ 2, 3, 5 })) == 1.f);
	CHECK(det(Yt::Matrix4::rotation(37, { 2, 3, 5 })) == Approx{ 1.f }.epsilon(2e-5));
	CHECK(det(Yt::Matrix4::scaling(4) * Yt::Matrix4::rotation(37, { 2, 3, 5 }) * Yt::Matrix4::scaling(2) * Yt::Matrix4::translation({ 2, 3, 5 })) == Approx{ 4.f * 4.f * 4.f * 2.f * 2.f * 2.f }.epsilon(2e-5));
}

TEST_CASE("matrix4.euler")
{
	const Yt::Matrix4 actual{ Yt::Euler{ 30, 45, 60 } };
	const auto expected = Yt::Matrix4::rotation(30, { 0, 0, -1 }) * Yt::Matrix4::rotation(45, { 1, 0, 0 }) * Yt::Matrix4::rotation(60, { 0, 1, 0 });
	CHECK(actual.x.x == Approx{ expected.x.x }.epsilon(3e-5));
	CHECK(actual.x.y == Approx{ expected.x.y }.epsilon(3e-5));
	CHECK(actual.x.z == Approx{ expected.x.z }.epsilon(3e-5));
	CHECK(actual.x.w == Approx{ expected.x.w }.epsilon(3e-5));
...
{
	CHECK(inverse(Yt::Matrix4::identity()) == Yt::Matrix4::identity());
	CHECK(inverse(Yt::Matrix4::translation({ 2, 3, 5 })) == Yt::Matrix4::translation({ -2, -3, -5 }));
	CHECK(inverse(Yt::Matrix4::scaling(4)) == Yt::Matrix4::scaling(0.25));
	{
		const auto actual = inverse(Yt::Matrix4::rotation(37, { 2, 3, 5 }));
		const auto expected = Yt::Matrix4::rotation(-37, { 2, 3, 5 });
	...
	{
		const auto actual = inverse(Yt::Matrix4::scaling(4) * Yt::Matrix4::rotation(37, { 2, 3, 5 }) * Yt::Matrix4::scaling(2) * Yt::Matrix4::translation({ 2, 3, 5 }));
		const auto expected = Matrix4::translation({ -2, -3, -5 }) * Yt::Matrix4::scaling(0.5) * Yt::Matrix4::rotation(-37, { 2, 3, 5 }) * Yt::Matrix4::scaling(0.25);
		CHECK(actual.x.x == Approx{ expected.x.x }.epsilon(2e-5));
		CHECK(actual.x.y == Approx{ expected.x.y }.epsilon(2e-5));
		CHECK(actual.x.z == Approx{ expected.x.z }.epsilon(2e-5));
		CHECK(actual.x.w == Approx{ expected.x.w }.epsilon(2e-5));
...
{
	const auto m = Yt::Matrix4::perspective({ 1, 1 }, 90, 1, 2);
	{
		const auto v = m * Yt::Vector3{ -1, -1, -1 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ -1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ -2, -2, -2 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ 1.f }.epsilon(2e-5));
	}
...
{
	const auto m = Yt::Matrix4::projection_2d({ 640, 480 }, -.25, .75);
	{
		const auto v = m * Yt::Vector3{ 0, 0, -.25 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ 1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 0, 0, .75 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ -1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 640, 0, -.25 };
		CHECK(v.x == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ 1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 640, 0, .75 };
		CHECK(v.x == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ -1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 0, 480, -.25 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ 1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 0, 480, .75 };
		CHECK(v.x == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ -1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 640, 480, -.25 };
		CHECK(v.x == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ 1.f }.epsilon(2e-5));
	}
	{
		const auto v = m * Yt::Vector3{ 640, 480, .75 };
		CHECK(v.x == Approx{ 1.f }.epsilon(2e-5));
		CHECK(v.y == Approx{ -1.f }.epsilon(2e-5));
		CHECK(v.z == Approx{ -1.f }.epsilon(2e-5));
	}
