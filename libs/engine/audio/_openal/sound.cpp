...
namespace Yttrium
{
		...
		: _format{ reader.format() }
	{
		...
		::alBufferData(_buffers[0], _format._format, buffer.data(), static_cast<ALsizei>(buffer.size()), _format._frequency);
		if (::alGetError() != AL_NO_ERROR)
			throw std::runtime_error{ "Failed to initialize a Sound" };
		::alSourcei(_source, AL_BUFFER, static_cast<ALint>(_buffers[0])); // This must be done after alBufferData.
