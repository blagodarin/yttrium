...
namespace Yt
{
	...
	public:
		...
		seir::RectF add(const seir::SizeF& = {}) noexcept;
		...
		[[nodiscard]] seir::RectF map(const seir::RectF&) const noexcept;
		constexpr void setAxis(Axis axis) noexcept { _axis = axis; }
		constexpr void setSize(const seir::SizeF& size) noexcept { _defaultSize = size; }
