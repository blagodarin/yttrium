...
namespace Yt
{
	...
	GuiLayout::GuiLayout(GuiFrame& frame, const Center& mapping) noexcept
		...
		, _previous{ std::exchange(frame._context._layout, this) }
	{
		const seir::SizeF viewport{ frame._context._window.size() };
	...
	seir::RectF GuiLayout::add(const seir::SizeF& size) noexcept
	{
		const auto rectSize = size.isNull() ? _defaultSize : size;
		...
		return seir::RectF{ { x1, y1 }, seir::Vec2{ x2, y2 } } * _scaling + _offset;
	}
	...
	seir::RectF GuiLayout::map(const seir::RectF& rect) const noexcept
	{
