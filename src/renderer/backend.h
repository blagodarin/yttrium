...
namespace Yttrium
{
	...
	public:
		...
		virtual std::unique_ptr<GpuProgram> create_builtin_program_2d(RendererImpl&) = 0;
		virtual std::unique_ptr<GpuProgram> create_gpu_program(RendererImpl&, const std::string& vertex_shader, const std::string& fragment_shader) = 0;
		virtual std::unique_ptr<Mesh> create_mesh(const MeshData&) = 0;
		virtual std::unique_ptr<Texture2D> create_texture_2d(RendererImpl&, Image&&, Flags<Renderer::TextureFlag>) = 0;
