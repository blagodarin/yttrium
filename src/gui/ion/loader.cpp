...
namespace Yttrium
{
	namespace
	{
		...
		{
		public:
			GuiException(String&& what) : _what(std::move(what)) {}
	...
	bool GuiIonLoader::load(const StaticString& source_name)
	{
		try
		{
			...
			if (!document.load(source_name))
				throw GuiException(String(&_allocator) << "Can't load \""_s << source_name << "\"..."_s);
			load(document.root());
			return true;
		}
	...
	void GuiIonLoader::load(const IonObject& source)
	{
		...
		for (const auto& node : source)
		{
			if (node.is_empty())
			{
				...
				if (!attribute)
					throw GuiException(String(&_allocator) << "Unknown attribute '"_s << node.name() << "'"_s);
			...
			else
			{
				...
				if (i == handlers.end())
					throw GuiException(String(&_allocator) << "Unknown entry '"_s << node.name() << "'"_s);
	...
	void GuiIonLoader::load_class(const IonNode& node, unsigned)
	{
		...
		if (!element.object || !element.name)
			throw GuiException(String(&_allocator) << "Bad 'class'"_s);
		if (!_classes.add(*element.name, *element.object, element.attribute))
			throw GuiException(String(&_allocator) << "Bad 'class' \""_s << *element.name << "\""_s);
	}
	...
	void GuiIonLoader::load_font(const IonNode& node, unsigned attributes)
	{
		...
		if (!element.object)
			throw GuiException(String(&_allocator) << "Bad 'font'"_s);
		...
		if (attributes & IsDefault)
		{
			if (_has_default_font)
				throw GuiException(String(&_allocator) << "Default 'font' redefinition"_s);
		...
		if (!GuiIonPropertyLoader::load_text(&font_name, element.object->last("file"_s)))
			throw GuiException(String(&_allocator) << "Bad 'font' 'file'"_s);

		const StaticString* texture_name;
		if (!GuiIonPropertyLoader::load_text(&texture_name, element.object->last("texture"_s)))
			throw GuiException(String(&_allocator) << "Bad 'font' 'texture'"_s);
	...
	void GuiIonLoader::load_include(const IonNode& node, unsigned)
	{
		...
		if (!Ion::get(node, path))
			throw GuiException(String(&_allocator) << "Bad 'include'"_s);
		load(*path);
	}
	...
	void GuiIonLoader::load_layer(const IonNode& node, unsigned attributes)
	{
		static const std::map<StaticString, std::pair<void (GuiIonLoader::*)(GuiLayer&, const IonNode&, int) const, int>> handlers =
		{
			...
			{"on_pop"_s, {&GuiIonLoader::load_layer_on_pop, 0}},
			{"on_push"_s, {&GuiIonLoader::load_layer_on_push, 0}},
			{"stretch"_s, {&GuiIonLoader::load_layer_layout, static_cast<int>(GuiLayout::Placement::Stretch)}},
		};

		const auto& element = load_element(node);
		if (!element.object)
			throw GuiException(String(&_allocator) << "Empty 'layer'"_s);
		...
		if (!is_root && layer_name.is_empty())
			throw GuiException(String(&_allocator) << "Unnamed non-root 'layer'"_s);

		auto layer = _gui.create_layer(layer_name, attributes == IsTransparent);
		if (!layer)
			throw GuiException(String(&_allocator) << "Bad 'layer'"_s);

		for (const auto& node : *element.object)
		{
			const auto i = handlers.find(node.name());
			if (i == handlers.end())
				throw GuiException(String(&_allocator) << "Unknown 'layer' entry '"_s << node.name() << "'"_s);
			(this->*i->second.first)(*layer, node, i->second.second);
		}
	...
	void GuiIonLoader::load_layer_layout(GuiLayer& layer, const IonNode& node, int extra) const
	{
		...
		for (const auto& layout_node : *node.first()->object())
		{
			...
			if (!element.object)
				throw GuiException(String(&_allocator) << "Bad 'layout' entry '"_s << layout_node.name() << "'"_s);
			...
			const auto widget = layout.add_widget(layout_node.name(), loader);
			if (!widget)
				throw GuiException(String(&_allocator) << "Unknown widget '"_s << node.name() << "'"_s);

			if (element.name)
				widget->set_name(*element.name);

			if (!layer.add_widget(widget))
				throw GuiException(String(&_allocator) << "Duplicate widget name \""_s << widget->name() << "\""_s);
		}
	}
	...
	void GuiIonLoader::load_layer_on_pop(GuiLayer& layer, const IonNode& node, int) const
	{
		const StaticString* action;
		if (GuiIonPropertyLoader::load_text(&action, node))
			layer.set_pop_action(ScriptCode(*action, &_allocator));
	}

	void GuiIonLoader::load_layer_on_push(GuiLayer& layer, const IonNode& node, int) const
	{
		const StaticString* action;
		if (GuiIonPropertyLoader::load_text(&action, node))
			layer.set_push_action(ScriptCode(*action, &_allocator));
	}
